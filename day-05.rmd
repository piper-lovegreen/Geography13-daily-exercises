...
```{r}
covid %>% 
  select(date, county, deaths, cases) %>%
  filter(!cases == 0) %>% 
  filter(!county == "Unknown") %>% 
  filter(date== max(date)) %>% 
  mutate(Ratio_Deaths_Cases= 100* (deaths/cases)) %>% 
  slice_max(Ratio_Deaths_Cases, n=5)
```
...
covid %>% 
  ...
  filter(date== max(date)) %>% 
  group_by(state) %>% 
  summarise(Ratio_Deaths_Cases= 100* (sum(deaths)/sum(cases))) %>% 
  slice_max(Ratio_Deaths_Cases, n=5)
